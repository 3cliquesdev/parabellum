-- Atualizar RPC para evitar duplicação de emails
-- Ignora leads cujo email da Kiwify já existe em outro contato
...
BEGIN
  ...
  WHERE ct.status = 'lead'
    ...
    AND c.status = 'open'
    -- NOVO: Ignorar se o email já existe em outro contato
    AND NOT EXISTS (
