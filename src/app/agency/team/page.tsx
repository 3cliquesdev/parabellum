    ...
    <div className="p-8 space-y-6 max-w-2xl" style={{ fontFamily: "var(--font-sans)" }}>
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "#939da4" }}>Membros da sua agencia com acesso ao painel</p>
        </div>
        <button onClick={() => setShowInvite(!showInvite)} className="flex items-center gap-2 px-4 h-9 rounded-xl text-sm font-bold" style={{ background: "rgba(154,234,98,0.1)", color: "#9aea62", border: "1px solid rgba(154,234,98,0.2)" }}>
      ...
      <div className="rounded-2xl p-6 space-y-3" style={cardStyle}>
        ...
        {members.length === 0 ? (
          <p className="text-xs py-4 text-center" style={{ color: "#939da4" }}>Nenhum membro alem de voce</p>
        ) : (
          members.map((member) => (
            <div key={member.id} className="flex items-center justify-between py-2.5 px-3 rounded-xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)" }}>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: "rgba(154,234,98,0.1)", color: "#9aea62" }}>
                  {member.user_id.slice(0, 2).toUpperCase()}
                </div>
                <div>
                  <p className="text-xs font-medium text-white">{member.user_id.slice(0, 8)}...</p>
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ color: ROLE_COLOR[member.role], background: `${ROLE_COLOR[member.role]}15` }}>
                  {ROLE_LABEL[member.role]}
                </span>
                {member.role !== "owner" && (
                  <button>
                    <Trash2 className="w-3.5 h-3.5" style={{ color: "rgba(248,113,113,0.4)" }} />
                  </button>
                )}
              </div>
            </div>
