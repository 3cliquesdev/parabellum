...
BEGIN
  ...
  WITH base AS (
    ...
    LEFT JOIN departments d ON d.id::text = c.department
  ...
  metrics AS (
    SELECT
      b.*,
      EXTRACT(EPOCH FROM (b.afm - b.cfm)) AS ai_fr_sec,
      EXTRACT(EPOCH FROM (b.ho_at - b.afm)) AS ai_dur_sec,
      EXTRACT(EPOCH FROM (b.ho_at - b.cfm)) AS tth_sec,
      EXTRACT(EPOCH FROM (b.agfm - b.ho_at)) AS hp_sec,
      EXTRACT(EPOCH FROM (b.res_at - b.agfm)) AS hr_sec,
      EXTRACT(EPOCH FROM (b.res_at - b.cfm)) AS tr_sec
    FROM base b
  ),
  agg AS (
    SELECT
      COUNT(*)::bigint AS tc,
      AVG(ai_fr_sec) FILTER (WHERE ai_fr_sec > 0) AS avg_ai_fr,
      AVG(ai_dur_sec) FILTER (WHERE ai_dur_sec > 0) AS avg_ai_dur,
      AVG(hp_sec) FILTER (WHERE hp_sec > 0) AS avg_hp,
      AVG(hr_sec) FILTER (WHERE hr_sec > 0) AS avg_hr,
      AVG(tr_sec) FILTER (WHERE tr_sec > 0) AS avg_tr,
      PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY ai_fr_sec) FILTER (WHERE ai_fr_sec > 0) AS p50_ai_fr,
      PERCENTILE_CONT(0.9) WITHIN GROUP (ORDER BY ai_fr_sec) FILTER (WHERE ai_fr_sec > 0) AS p90_ai_fr,
