...
-- ============================================================
-- FIX: Optimize commercial conversations RPCs
-- 1. Fix KPI timeout (indexed subquery instead of full JOIN on messages)
-- 2. Distinguish bot vs human (sender_id IS NOT NULL)
-- 3. Improve assigned_agent_name logic (Bot/IA/Agent/Não atribuído)
-- ============================================================

-- Index to speed up first-human-agent-message lookup (using enum value directly)
CREATE INDEX IF NOT EXISTS idx_messages_conv_human_sender
ON messages (conversation_id, created_at)
WHERE sender_type = 'user' AND sender_id IS NOT NULL;

-- ============================================================
-- 1. OPTIMIZED KPIs RPC
-- ============================================================
...
AS $function$
  WITH conv_base AS (
    SELECT c.id, c.status, c.created_at, c.closed_at, c.first_response_at
  ...
  conv_with_calcs AS (
    SELECT
      ...
      EXISTS (
        ...
        WHERE ct.conversation_id = cb.id AND t.category = 'conversation'
      ) AS has_tag,
      (
        SELECT MIN(m.created_at)
        FROM messages m
        WHERE m.conversation_id = cb.id
          AND m.sender_type::text IN ('agent', 'user')
          AND m.sender_id IS NOT NULL
      ) AS first_human_msg_at
    FROM conv_base cb
  )
  SELECT
    COUNT(*)::bigint AS total_conversations,
    COUNT(*) FILTER (WHERE cc.status::text = 'open')::bigint AS total_open,
    COUNT(*) FILTER (WHERE cc.status::text = 'closed')::bigint AS total_closed,
    COUNT(*) FILTER (WHERE NOT cc.has_tag)::bigint AS total_without_tag,
    (SELECT AVG(cr.rating)::numeric FROM conversation_ratings cr JOIN conv_base cb ON cb.id = cr.conversation_id) AS avg_csat,
    AVG(
      EXTRACT(EPOCH FROM (COALESCE(cc.first_human_msg_at, cc.first_response_at) - cc.created_at))
    ) FILTER (
      WHERE COALESCE(cc.first_human_msg_at, cc.first_response_at) IS NOT NULL
    ) AS avg_waiting_seconds,
    AVG(
      EXTRACT(EPOCH FROM (cc.closed_at - cc.created_at))
    ) FILTER (WHERE cc.closed_at IS NOT NULL) AS avg_duration_seconds
  FROM conv_with_calcs cc;
$function$;

-- ============================================================
-- 2. OPTIMIZED REPORT RPC
-- ============================================================
