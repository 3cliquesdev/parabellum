...
export function SLAComplianceWidgetV2() {
  ...
  const handleCategoryClick = (metric: string, label: string) => {
    if (!isLoading) {
      setDrilldownMetric({ metric, label });
    }
  };
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                <div 
                  className={cn(
                    "p-2 rounded-lg bg-green-50 dark:bg-green-950 transition-all",
                    "cursor-pointer hover:ring-2 hover:ring-green-500/30"
                  )}
                  onClick={() => handleCategoryClick("sla_on_time", "Tickets no Prazo")}
                >
                ...
                <div 
                  className={cn(
                    "p-2 rounded-lg bg-red-50 dark:bg-red-950 transition-all",
                    "cursor-pointer hover:ring-2 hover:ring-red-500/30"
                  )}
                  onClick={() => handleCategoryClick("sla_overdue", "Tickets Atrasados")}
                >
