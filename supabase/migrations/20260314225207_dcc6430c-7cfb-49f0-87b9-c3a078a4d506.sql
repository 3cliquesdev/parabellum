DROP FUNCTION IF EXISTS public.get_inbox_time_report(timestamptz, timestamptz, uuid, uuid, text, text, uuid, text, text, integer, integer);
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      (SELECT MIN(m.created_at) FROM messages m WHERE m.conversation_id = c.id AND m.sender_type = 'user' AND m.is_ai_generated = false AND c.handoff_executed_at IS NOT NULL AND m.created_at > c.handoff_executed_at) AS agfm,
