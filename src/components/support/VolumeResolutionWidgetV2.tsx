      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium flex items-center justify-between">
            <span>Volume vs Resolucao</span>
            {!isLoading && (
              <div className="flex gap-4 text-xs">
                <span 
                  className="text-blue-600 dark:text-blue-400 cursor-pointer hover:underline"
                  onClick={() => setDrilldownMetric({ metric: "volume_opened", label: "Tickets Abertos" })}
                >
                  Abertos: {totalOpened}
                </span>
                <span 
                  className="text-green-600 dark:text-green-400 cursor-pointer hover:underline"
                  onClick={() => setDrilldownMetric({ metric: "volume_resolved", label: "Tickets Resolvidos" })}
                >
