...
async function fetchInstagramUsername(instagramBusinessAccountId: string, accessToken: string) {
  try {
    const response = await fetch(`https://graph.facebook.com/v20.0/${instagramBusinessAccountId}?fields=username&access_token=${accessToken}`);
...
export async function POST(request: NextRequest) {
  ...
  const username = await fetchInstagramUsername(body.instagram_business_account_id, body.access_token);
