...
DROP FUNCTION IF EXISTS get_commercial_conversations_report(timestamptz, timestamptz, uuid, uuid, text, text, text, int, int);

CREATE OR REPLACE FUNCTION get_commercial_conversations_report(
  p_start timestamptz,
  p_end timestamptz,
...
AS $$
  ...
  LEFT JOIN LATERAL (
    SELECT
      ARRAY_AGG(DISTINCT t.name ORDER BY t.name) FILTER (
        WHERE NOT EXISTS (
          SELECT 1 FROM protected_conversation_tags pct WHERE pct.tag_id = t.id
        )
      ) AS tags_all,
      ARRAY_AGG(DISTINCT t.name ORDER BY t.name) FILTER (
        WHERE EXISTS (
